...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/dashboard'); // Or use hook for redirect
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      <form onSubmit={handleSignup} className="p-6 bg-white rounded shadow-md">
        ...
        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" className="w-full p-2 mb-4 border" />
        ...
        <button type="submit" className="w-full p-2 text-white bg-blue-500">Signup</button>
      </form>
    </div>
  );
}